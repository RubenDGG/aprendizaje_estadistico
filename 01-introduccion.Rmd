...
Se trata de un campo multidisciplicar, con importantes aportaciones estadísticas , conocimiento del negocio e informáticas, dentro del que se incluirían disciplinas como *Minería de Datos* (Data Mining), *Aprendizaje Automático* (Machine Learning), *Aprendizaje Profundo* (Deep Learning), *Modelado Predictivo* (Predictive Modeling), *Extracción de Conocimiento* (Knowlegde Discovery) y también el *Aprendizaje Estadístico* (Statistical Learning).

Podríamos definir la Ciencia de Datos como el conjunto de conocimientos y herramientas utilizados en las distintas etapas del análisis de datos `r citefig("esquema")`. Otras definiciones podrían ser:
...
### Las dos culturas [@breiman2001statistical]
...
Reciprocamente, empleando su inversa, la *función logit*:
...
Entre ellos estarían R-Commander con el plugin FactoMineR (`Rcmdr`, `RcmdrPlugin.FactoMineR`), `r citepkg("rattle", "https://rattle.togaware.com")` y `r citepkg("radiant", "https://github.com/radiant-rstats/radiant")`.
...
La recomendación sería por tanto seleccionar los hiperparámetros (el modelo final) tratando de que haya un equilibrio entre el sesgo y la varianza `r citefig("biasvar")`.

```{r biasvar, echo=FALSE, out.width="80%", fig.cap="Equilibrio entre sesgo y varianza"}
...
Continuando con el ejemplo anterior, supongamos que queremos emplear regresión polinómica para explicar la valoración de las viviendas a partir del "estatus" de los residentes `r citefig("boston-mass")`. 
Al igual que se hizo en la Sección \@ref(bias-variance), consideraremos el grado del polinomio como un hiperparámetro.

```{r boston-mass, fig.cap='Gráfico de dispersión de las valoraciones de las viviendas (`medv`) frente al porcentaje de población con "menor estatus" (`lstat`).'}
...
```{r cv-mse, fig.cap="Error cuadrático medio de validación cruzada dependiendo del grado del polinomio (complejidad) y valor óptimo."}
grado.max <- 10
grados <- seq_len(grado.max) 
cv.mse <- cv.mse.sd <- numeric(grado.max)
for(grado in grados){
  # cv.res <- cv.lm0(medv ~ poly(lstat, grado), train) # Tiempo de computación elevado!
...
```{r cv-onese, fig.cap="Error cuadrático medio de validación cruzada dependiendo del grado del polinomio (complejidad) y valor seleccionado con el criterio de un error estándar."}
plot(grados, cv.mse, ylim = c(25, 45),
  xlab = "Grado del polinomio")
...
```{r boston-final, fig.cap='Ajuste de los modelos finales, empleando el valor óptimo y el criterio de un error estándar para seleccionar el grado del polinomio mediante validación cruzada.'}
plot(medv ~ lstat, data = train)
...
Si generamos un gráfico de dispersión de observaciones frente a predicciones, los puntos deberían estar en torno a la recta $y=x$ `r citefig("obs-pred-plot")`.
...
```{r obs-pred-plot, fig.cap="Gráfico de dispersión de observaciones frente a predicciones (incluyendo la identidad, línea continua, y el ajuste lineal, línea discontinua)."}
obs <- test$medv
...
```{r featureplot, fig.cap="Distribución del estatus de la población dependiendo del nivel de valoración de las viviendas."}
# data(Boston, package = "MASS")
datos <- Boston
...
```{r classprob, fig.cap="Distribución de las probabilidades estimadas de valoración alta de la vivienda dependiendo de la categoría observada."}
...
```{r pfrontera, fig.cap='Proporción de "frontera" dependiendo del número de dimensiones y del porcentaje de valores considerados extremos en cada dimensión.'}
...
* [`ICEbox`](https://CRAN.R-project.org/package=ICEbox) ICEbox: Individual Conditional Expectation Plot Toolbox. <!-- Herramientas para visualizar el gráfico ICE y el modelo estimado por cualquier algoritmo de aprendizaje supervisado -->
...
* [`randomForestExplainer`](https://modeloriented.github.io/randomForestExplainer): Explaining and Visualizing Random Forests in Terms of Variable Importance. <!--conjunto de herramientas para explicar y visualizar los modelos de bosque aleatorio  -->
...
Como ya se comentó en la Sección \@ref(metodos-pkgs), el paquete `caret` [*Classification And REgression Training*; @kuhn2008building] proporciona una interfaz unificada que simplifica el proceso de modelado empleando la mayoría de los métodos de AE implementados en R (actualmente admite 239 métodos; ver el [Capítulo 6](https://topepo.github.io/caret/available-models.html) del [manual](https://topepo.github.io/caret) de este paquete). 
...
```{r fig.cap="Raíz del error cuadrático medio de validación cruzada dependiendo del valor del hiperparámetro."}
set.seed(1)
...
este paquete ha dejado de desarrollarse de forma activa, aunque podemos considerar que la alternativa `r citepkg("tidymodels", "https://www.tidymodels.org")` todavía está en una fase inicial de desarrollo.
Este es uno de los motivos por los que se ha optado por mantener el uso de `caret` en este libro, aunque la intención es incluir apéndices adicionales en próximas versiones ilustrando el uso de otras herramientas [como `tidymodels`, @kuhn2020tidymodels; o incluso `mlr3`, @becker2021mlr3].
